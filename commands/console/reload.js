...
exports.run = async (clients) => {
    ...
    clients.twitch.permittedlinks = readFileSync("./configs/links.txt", "utf8").split(/\r\n|\n\r|\n|\r/).filter((link) => link !== "");
    clients.twitch.deletelinks = readFileSync("./configs/TLDs.txt", "utf8").split(/\r\n|\n\r|\n|\r/).filter((link) => link !== "");
    clients.logger.debug("Folgende Links sind jetzt erlaubt: " + clients.twitch.permittedlinks.join("\t"));
    clients.logger.log("info", "Die automessages.json, TLDs.txt und links.txt wurden erfolgreich neu eingelesen.");
};
/**
 * @param  {import("../../modules/clients").Clients} clients
 * @param  {string} line
