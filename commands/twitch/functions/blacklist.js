...
exports.run = async (client, target, context) => {
    const bmsg = await client.clients.discord.blchannel.send(`In der Blacklist für ${target} sind die Wörter \
    ...
    await bmsg.react("🔄");
    const coll = bmsg.createReactionCollector({ filter: (r, u) => r.emoji.name == "🔄" && u.id !== client.clients.discord.user.id });
